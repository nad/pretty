...
module _ where
  ...
  -- The pretty-printer interface, Doc′, is defined for basic
  -- grammars. However, "extended" grammars can (of course) also be
  -- used: see Doc.
  ...
  mutual
    ...
    data Doc′ : ∀ {A} → Grammar A → A → Set₁ where
      ...
      line  : Doc (tt G.<$ G.whitespace G.+) tt
      ...
      fill  : ∀ {A xs} {g : G.Grammar A} →
              Docs g xs → Doc (g G.sep-by G.whitespace G.+) xs

    -- Sequences of documents, all based on the same grammar.

    data Docs {A} (g : G.Grammar A) : List⁺ A → Set₁ where
...
-- "Smart" variants of emb. Can be used to avoid long chains of emb
-- constructors.

embed′ : ∀ {A B} {g₁ : Basic.Grammar A} {g₂ : Basic.Grammar B} {x y} →
         (∀ {s} → x Basic.∈ g₁ ∙ s → y Basic.∈ g₂ ∙ s) →
         Doc′ g₁ x → Doc′ g₂ y
...
embed f d = embed′ (sound ∘ f ∘ complete) d
...
symbol-line-doc = final-line {g = symbol _} 0 1 symbol-doc
...
record Renderer : Set₁ where
  field
    ...
    render : ∀ {A} {g : Basic.Grammar A} {x} → Doc′ g x → List Char

    -- The rendered string must be parsable.

    parsable : ∀ {A} {g : Basic.Grammar A} {x}
               (d : Doc′ g x) → x Basic.∈ g ∙ render d
  ...
  pretty-printer-correct pretty x = complete (parsable (pretty x))
...
ugly-renderer = record
  ...
  mutual
    ...
    parsable : ∀ {A x} {g : Basic.Grammar A}
               (d : Doc′ g x) → x Basic.∈ g ∙ render d
    parsable nil        = sound return-sem
    parsable text       = sound string-sem
    parsable (d₁ · d₂)  = Basic.>>=-sem (parsable d₁) (parsable d₂)
    parsable line       = sound (<$-sem single-space-sem)
    parsable (group d)  = parsable d
    parsable (nest _ d) = parsable d
    parsable (emb f d)  = f (parsable d)
    parsable (fill ds)  = sound (parsable-fills ds)

    parsable-fills : ∀ {A xs} {g : Grammar A} (ds : Docs g xs) →
                     xs ∈ g sep-by whitespace + ∙ render-fills ds
    parsable-fills [ d ]    = sep-by-sem-singleton
                                (complete (parsable d))
    parsable-fills (d ∷ ds) =
      sep-by-sem-∷ (complete (parsable d))
                   single-space-sem
                   (parsable-fills ds)
